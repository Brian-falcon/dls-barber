...
type ReservationEmail = { to: string; name: string; barber: string; service: string; date: Date; time: string; status: "PENDIENTE" | "CONFIRMADA" | "CANCELADA" | "REPROGRAMADA" };
...
export async function sendReservationEmail({ to, name, barber, service, date, time, status }: ReservationEmail) {
  ...
  const copy = status === "PENDIENTE" ? { title: "Recibimos tu reserva", subject: "Reserva recibida · DLS BARBER", intro: "Tu solicitud fue registrada y está pendiente de confirmación del profesional." } : status === "CONFIRMADA" ? { title: "Tu turno está confirmado", subject: "Reserva confirmada · DLS BARBER", intro: "Tu reserva fue confirmada. Te esperamos." } : status === "CANCELADA" ? { title: "Tu turno fue cancelado", subject: "Reserva cancelada · DLS BARBER", intro: "Tu reserva fue cancelada. Podés crear un nuevo turno cuando quieras." } : { title: "Tu turno fue reprogramado", subject: "Reserva reprogramada · DLS BARBER", intro: "La fecha u horario de tu reserva fue actualizado." };

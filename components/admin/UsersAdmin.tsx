...
export default function UsersAdmin({ initial }: { initial: UserData[] }) {
  ...
  return <div className="space-y-2">{users.map((user) => <div key={user.id} className="flex items-center justify-between gap-3 rounded border border-gray-800 bg-gray-900 p-3"><div className="min-w-0"><div className="truncate text-sm text-white">{user.nombre ?? user.email}</div><div className="truncate text-sm text-gray-400">{user.email}</div></div><div className="flex items-center gap-2"><select disabled={savingId === user.id} value={user.rol} onChange={(event) => changeRole(user.id, event.target.value as UserData["rol"])} className="rounded border border-gray-700 bg-black p-1 text-sm text-white disabled:opacity-50"><option value="CLIENTE">CLIENTE</option><option value="BARBERO">BARBERO</option><option value="ADMIN">ADMIN</option></select>{user.rol === "CLIENTE" && <button disabled={savingId === user.id} onClick={() => removeClient(user)} className="rounded border border-rose-500/50 px-2 py-1 text-xs text-rose-300 disabled:opacity-50">Eliminar</button>}</div></div>)}</div>;
}

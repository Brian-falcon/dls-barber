  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">{items.map((barber) => {
        ...
        return <div key={barber.id} className="rounded-xl border border-white/10 bg-white/[0.03] p-3"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium text-white">{barber.nombre}</p><p className="text-sm text-slate-400">{barber.activo ? "Disponible" : "No disponible"} · {barber.user ? barber.user.email : "Sin cuenta asignada"}</p></div><div className="flex flex-wrap gap-2"><select disabled={busyId === barber.id} value={barber.userId ?? ""} onChange={(event) => update(barber.id, `/api/admin/barbers/${barber.id}/assign`, { userId: event.target.value || null })} className="rounded-lg border border-white/10 bg-black px-2 py-1 text-sm text-white disabled:opacity-50"><option value="">Sin asignar</option>{eligibleStaff.map((user) => <option key={user.id} value={user.id}>{user.nombre} · {user.email}</option>)}</select><button disabled={busyId === barber.id} onClick={() => update(barber.id, `/api/admin/barbers/${barber.id}/toggle`)} className="rounded-lg bg-white/10 px-3 py-1 text-sm disabled:opacity-50">{barber.activo ? "Desactivar" : "Activar"}</button></div></div></div>;
      })}</div>
    </div>
  );
}

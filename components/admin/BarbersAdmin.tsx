...
export default function BarbersAdmin({ initial, staff }: { initial: Barber[]; staff: StaffUser[] }) {
  ...
  return <div className="space-y-4"><form onSubmit={createBarber} className="flex flex-col gap-2 sm:flex-row"><input required minLength={2} value={name} onChange={(event) => setName(event.target.value)} placeholder="Nombre del profesional" className="flex-1 rounded-xl border border-white/10 bg-black/40 px-3 py-2" /><button className="rounded-xl bg-[var(--gold)] px-4 py-2 font-semibold text-black">Agregar profesional</button></form><p className="text-xs text-slate-400">Primero registrá la cuenta del profesional, cambiá su rol a BARBERO y luego asignala aquí.</p><div className="space-y-2">{items.map((barber) => <div key={barber.id} className="rounded-xl border border-white/10 bg-white/[0.03] p-3"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium text-white">{barber.nombre}</p><p className="text-sm text-slate-400">{barber.activo ? "Disponible" : "No disponible"} · {barber.user ? barber.user.email : "Sin cuenta asignada"}</p></div><div className="flex flex-wrap gap-2"><select value={barber.userId ?? ""} onChange={(event) => update(barber.id, `/api/admin/barbers/${barber.id}/assign`, { userId: event.target.value || null })} className="rounded-lg border border-white/10 bg-black px-2 py-1 text-sm text-white"><option value="">Sin asignar</option>{staff.map((user) => <option key={user.id} value={user.id}>{user.nombre} · {user.email}</option>)}</select><button onClick={() => update(barber.id, `/api/admin/barbers/${barber.id}/toggle`)} className="rounded-lg bg-white/10 px-3 py-1 text-sm">{barber.activo ? "Desactivar" : "Activar"}</button></div></div></div>)}</div></div>;
}
